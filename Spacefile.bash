...
SPACE_INSTALL_BIN()
{
    ...
    if [ "$?" -gt 0 ]; then
        if [ "$(id -u)" != 0 ]; then
            PRINT "This program must be run as root in order to install ${_bin_file_name} to ${_bindest}. If performing the manual install, please run: \"sudo ./space /install/\". Otherwise just run the automated install again with \"curl https://get.space.sh | sudo sh\"" "error"
        fi
